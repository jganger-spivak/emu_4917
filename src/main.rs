...
fn load_from_file(mem: &mut [u8], filename: &String) {
    ...
    match result {
        Ok(data) => {
            code = String::from_utf8_lossy(&data).parse::<String>().unwrap();
        }
        Err(err) => {
            panic!("Couldn't read file: {}", err.to_string())
        }
    }
    let mut line_num = 0;
...
fn execute(halt: &mut bool, ip: &mut usize, r0: &mut u8, r1: &mut u8, mem: &mut [u8; MEM_MAX]) {
    ...
    match mem[*ip] {
        ...
        16_u8..=u8::MAX => {
            warn!(
                "Non-implemented opcodes are being executed at IP: {}, check your jumps",
                *ip
            );
            *ip = ip_plus_one;
        }
